/*
AI-Assisted Code (Schema Design)

Prompt: Review this users table schema and suggest improvements for data integrity, 
security best practices, and constraints for a marketplace application.

Additional Notes: I designed the initial table with the core fields like id, email, 
name, and role. AI suggested adding UNIQUE on email and phone_number, DEFAULT values 
for the boolean flags, and the is_suspended field for moderation.
*/
-- 1. USERS 
...
Prompt: What CHECK constraints should I add to a listings table to ensure valid 
enum values for condition, category, and status fields?

Additional Notes: I created the base listings table with the required fields. AI 
helped me add the CHECK constraints for the enums and suggested the search_vector 
column with weighted full-text search so title matches rank higher than description.
*/
-- 2. LISTINGS 
...
BEGIN
  NEW.search_vector := 
    setweight(to_tsvector('english', lower(COALESCE(NEW.title, ''))), 'A') ||
    setweight(to_tsvector('english', lower(COALESCE(NEW.description, ''))), 'B');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
Prompt: What report reason categories should a marketplace application support, 
and how should I structure the status workflow?

Additional Notes: I designed the reports table structure. AI suggested the reason 
values like scam, prohibited, harassment based on common marketplace patterns and 
the status workflow going from open to in_review to resolved.
*/
-- 4. REPORTS
